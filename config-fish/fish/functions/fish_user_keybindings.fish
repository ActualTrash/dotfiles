function fish_vi_key_bindings_custom --description 'colemak approved vi-like key bindings for fish'
    if contains -- -h $argv
    ...
    bind -s --preset -M default i forward-char # CO: l -> i
    bind -s --preset -m insert \n execute
    bind -s --preset -m insert \r execute
    bind -s --preset -m insert y insert-line-under repaint-mode # CO: o -> y
    bind -s --preset -m insert Y insert-line-over repaint-mode # CO: O -> Y
    bind -s --preset -m insert u repaint-mode # CO: i -> u
    bind -s --preset -m insert U beginning-of-line repaint-mode # CO: I -> U
    ...
    bind -s --preset dd beginning-of-buffer # CO: gg -> dd
    bind -s --preset D end-of-buffer # CO: G -> D
    ...
    bind -s --preset l undo # CO: u -> l
    bind -s --preset \cr redo

    bind -s --preset [ history-token-search-backward
    bind -s --preset ] history-token-search-forward

    bind -s --preset e up-or-search # CO: k -> e
    bind -s --preset n down-or-search # CO: j -> n
    bind -s --preset b backward-word
    bind -s --preset B backward-bigword
    bind -s --preset df backward-word # CO: ge -> df
    bind -s --preset dF backward-bigword # CO: gE -> dF
    bind -s --preset w forward-word forward-single-char
    bind -s --preset W forward-bigword forward-single-char
    bind -s --preset f forward-single-char forward-word backward-char # CO: e -> f
    bind -s --preset F forward-bigword backward-char # CO: e -> F
    ...
    bind -s --preset ss kill-whole-line # CO: dd -> ss
    bind -s --preset S kill-line # CO: D -> S
    bind -s --preset s\$ kill-line # CO: d\$ -> s\$
    bind -s --preset s\^ backward-kill-line # CO: d\^ -> s\^
    bind -s --preset s0 backward-kill-line # CO: d0 -> s0
    bind -s --preset sw kill-word # CO: dw -> sw
    bind -s --preset sW kill-bigword # CO: dW -> sW
    bind -s --preset suw forward-single-char forward-single-char backward-word kill-word # CO: diw -> suw
    bind -s --preset suw forward-single-char forward-single-char backward-bigword kill-bigword # CO: diW -> suW
    bind -s --preset saw forward-single-char forward-single-char backward-word kill-word # CO: daw -> saw
    bind -s --preset saW forward-single-char forward-single-char backward-bigword kill-bigword # CO: daW -> saW
    bind -s --preset sf kill-word # CO: de -> sf
    bind -s --preset sF kill-bigword # CO: dF -> sF
    bind -s --preset sb backward-kill-word # CO: db -> sb
    bind -s --preset sB backward-kill-bigword # CO: dB -> sB
    bind -s --preset sdf backward-kill-word # CO: dge -> sdf
    bind -s --preset sdF backward-kill-bigword # CO: dgE -> sdF
    bind -s --preset st begin-selection forward-jump kill-selection end-selection # CO: df -> st
    bind -s --preset sg begin-selection forward-jump backward-char kill-selection end-selection # CO: dt -> sg
    bind -s --preset sT begin-selection backward-jump kill-selection end-selection # CO: dF -> sT
    bind -s --preset sG begin-selection backward-jump forward-single-char kill-selection end-selection # CO: dT -> sG
    bind -s --preset sh backward-char delete-char # CO: dh -> sh
    bind -s --preset si delete-char # CO: dl -> si
    bind -s --preset su backward-jump-till and repeat-jump-reverse and begin-selection repeat-jump kill-selection end-selection # di -> su
    bind -s --preset sa backward-jump and repeat-jump-reverse and begin-selection repeat-jump kill-selection end-selection # CO: da -> sa
    bind -s --preset 's;' begin-selection repeat-jump kill-selection end-selection # CO: d; -> s;
    bind -s --preset 's,' begin-selection repeat-jump-reverse kill-selection end-selection # CO: d, -> s,
    #      <--- Colemak Pointer (this is where we left off)
    ...
    bind -s --preset -M visual i forward-char # CO: l -> i

    bind -s --preset -M visual e up-line # CO: k -> e
    bind -s --preset -M visual n down-line # CO: j -> n
    ...
    bind -s --preset -M visual -m insert r kill-selection end-selection repaint-mode # CO: s -> r
    bind -s --preset -M visual -m default s kill-selection end-selection repaint-mode # CO: d -> s
    ...
    bind -s --preset -M visual -m default j kill-selection yank end-selection repaint-mode # CO: y -> j
    bind -s --preset -M visual -m default '"*j' "fish_clipboard_copy; commandline -f end-selection repaint-mode" # CO: "*y -> "*j
...
function fish_user_key_bindings
    #echo 'Loaded keybinds'
    fish_vi_key_bindings_custom

    # Bind pf and fp to transition to normal mode
    bind -M insert -m default fp backward-char force-repaint
    bind -M insert -m default pf backward-char force-repaint
end
